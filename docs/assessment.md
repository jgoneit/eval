...
creates an absent destination directory containing `assessment.json` and
`report.md`. Existing directories are never replaced, including after an
...
read-only and defaults to JSON. Neither command reads or appends an experiment
...
| [eval-attempts/v1](../schemas/attempts-v1.schema.json) | Suite identity; instruction condition; model/reasoning/permission/environment/tool identity; attempts; final file manifest; checks; normalized events; optional measurements |
| [eval-assessment/v1](../schemas/assessment-v1.schema.json) | Evaluator version; case outcomes; process findings; sanitized evidence references and origins; coverage; observed metrics; planned denominator |
| [eval-comparison/v1](../schemas/comparison-v1.schema.json) | Paired case changes; comparable and unevaluated populations; missing attempts; candidate-minus-baseline measurements and their coverage |
...
are rejected. There is at most one attempt per case per condition in v1; the
protocol provides no best-retry selection. An absent attempt remains in the
planned population.

Suite manifests use normalized relative file paths. They are private input;
assessment and comparison outputs contain case IDs, safe metadata tokens,
...
reasoning, permission profile, environment, and tool digest. The instruction
digest is the intended varying factor. Incompatible assessments are rejected.
...
completed count as comparable. Missing, interrupted, and environment-failed
...
values, with per-metric sample counts. Missing values stay `null`. Observed
runtime differences do not establish time saved; concurrent load, caching,
